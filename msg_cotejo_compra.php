 ...
 */
    ...
    if(isset($confirmar)){
        ...
        foreach($_POST['id_pedido'] as $ids){
            mysql_query("UPDATE pedidos SET estado = 3, fechaHoraCotejado = now(), codigoUsuarioCotejado = '$codigo_usuario' WHERE idPedido IN ('$ids')",$link);
        }
    }
    if(isset($anular)){
        //************anular***************
    ...
    header("Location: cotejo_compra.php");
?>
<?php
    ob_end_flush();
?>