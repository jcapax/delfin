...
<html>
<head>
<title>EXISTENCIA ALMACEN</title>
...
<?
    ...
    if($flag == 1){
?>
<form action="existencia_almacen.php" name="form1" id="form1" method="post">
...
<form action="msg_existencia_almacen.php" method="post" name="form3">
<table width='900' border='0' id='tabla1'>
  <?
    $sql = "SELECT * FROM v_detallepedidos WHERE estado = 1 AND origen='ALMACEN'";
    
    if($_POST)
    {    
		if($estado == 1){
		  if($fecha1==''){
            if(strlen($filtro)>1){$sql = "SELECT * FROM v_detallepedidos WHERE estado = 1 AND CONCAT(nombreArticulo,seccion,solicitante) LIKE '%$filtro%' AND origen='ALMACEN'";}		    
		  }
          else{
            $sql = "SELECT * FROM v_detallepedidos WHERE estado = 1 AND fecha BETWEEN '$fecha1' AND '$fecha2' AND CONCAT(nombreArticulo,seccion,solicitante) LIKE '%$filtro%' AND origen='ALMACEN'";
          }
        }
        else {
		  if($fecha1==''){
            if(strlen($filtro)>1){$sql = "SELECT * FROM v_detallepedidos WHERE estado = 5 AND CONCAT(nombreArticulo,seccion,solicitante) LIKE '%$filtro%' AND origen='ALMACEN'";}		    
		  }
          else{
            $sql = "SELECT * FROM v_detallepedidos WHERE estado = 5 AND fecha BETWEEN '$fecha1' AND '$fecha2' AND CONCAT(nombreArticulo,seccion,solicitante) LIKE '%$filtro%' AND origen='ALMACEN'";
          }
        }
        //echo $sql;
        unset($fecha1);
        ...
        while($res_pedidos = mysql_fetch_array($sql_pedidos)){
            ...
            echo "<tr style='font-size:11px'>";
                ...
                if ($estado == 1){
                    echo "<td width='41' align= 'right'><input type='checkbox' value='$res_pedidos[0]' name='id_pedido[]'/></td>";
                }
