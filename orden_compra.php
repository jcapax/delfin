...
<?
    ...
    if($flag == 1){
?>
<form action="orden_compra.php" name="form1" id="form1" method="post">
...
<table width='900' border='0' id='tabla1'>
  <?
    $sql = "SELECT * FROM v_detallepedidos WHERE estado = 1 AND origen='COMPRA'";
    
    if($_POST)
    {    
		if($estado == 1){
		  if($fecha1==''){
            if(strlen($filtro)>1){$sql = "SELECT * FROM v_detallepedidos WHERE estado = 1 AND CONCAT(nombreArticulo,seccion,solicitante) LIKE '%$filtro%' AND origen='COMPRA'";}		    
		  }
          else{
            $sql = "SELECT * FROM v_detallepedidos WHERE estado = 1 AND fecha BETWEEN '$fecha1' AND '$fecha2' AND CONCAT(nombreArticulo,seccion,solicitante) LIKE '%$filtro%' AND origen='COMPRA'";
          }
        }
        else {
		  if($fecha1==''){
            if(strlen($filtro)>1){$sql = "SELECT * FROM v_detallepedidos WHERE estado = 2 AND CONCAT(nombreArticulo,seccion,solicitante) LIKE '%$filtro%' AND origen='COMPRA' AND origen='COMPRA'";}		    
		  }
          else{
            $sql = "SELECT * FROM v_detallepedidos WHERE estado = 2 AND fecha BETWEEN '$fecha1' AND '$fecha2' AND CONCAT(nombreArticulo,seccion,solicitante) LIKE '%$filtro%' AND origen='COMPRA'";
          }
        }
        //echo $sql;
